...
Shared-foundation milestone M2 is complete. Intent-core provides the persistent `ProjectDocument`, semantic patches and exact inverses, atomic undo/redo, revision guards, stable snapshots/subscriptions, and lossless validated JSON save/load. The React slice consumes that document as an external store, routes graph, inspector, diagnostic-suggestion, and board-placement edits through it, and exposes undo/redo plus browser load/save. Work now advances through the north-star semantic inventory and the first portable editor-core milestones while preserving this mutation boundary.
...
"Recreate" means functional and manufacturing equivalence against a content-pinned reference, not identical placement or trace geometry. The current reference is a useful scale marker (about 70 footprints, 123 observed nets, 379 pads, 611 track segments, 58 vias, and 5 zones), but these observations are not yet a qualified golden baseline. Provenance, licensing, source revision, known design issues, and the reference DRC result must be recorded before parity claims are accepted.
...
### G1 - North-star reference and semantic inventory - todo
